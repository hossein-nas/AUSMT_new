...
class FilesCategoryController extends Controller
{
    public function deleteCategory(Request $req){
        ...
        return back()->with('status','مشکلی در حذف بوجود آمد');
    ...
    public function editCategory(Request $req){
        ...
        if ( $it->save() ){
            return back()->with('status','با موفقیت ویرایش شد');
        }
    }
    ...
    public function addNewCategory(Request $req){
        ...
        if ($this->createDirInPath($new->base_dir_path)){
            $new->save();
            return back()->with('status','با موفقیت افزوده شد');
        }
        else
            return back()->with('status','خطایی رخ داد');
